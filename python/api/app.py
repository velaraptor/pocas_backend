...
api = Api(api_v1, version='v%s' % VERSION, title='POCAS API',
          description='an api to get results',
          )
...
top_results = api.model(
    'TopResults', {'id': fields.Integer(required=True, description="Unique id of Service", example=1),
                   ...
                   'pocas_score': fields.Float(required=True, description='score for ordering results, higher is better',
                                               example=0.12)
                   }
)
...
parser = api.parser()
parser.add_argument(
    "address", type=str, required=True, help="The task details", location="form"
)
parser.add_argument(
    "dob", type=str, required=True, help="The task details", location="form"
)
parser.add_argument(
    "question_answers", type=str, required=True, help="The task details", location="form"
)
